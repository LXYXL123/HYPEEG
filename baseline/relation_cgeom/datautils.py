...
def load_EEG_cls(dataset):
    dataset_path = os.path.join('eeg_processing', 'processed', f'{dataset}.npz')
    if not os.path.exists(dataset_path):
        raise FileNotFoundError(
            f'EEG dataset file not found: {dataset_path}. '
            f'Prepare it first, e.g. `python eeg_processing/preprocess_hgd.py` for HGD.'
        )
    ...
    if train_data.ndim != 3 or test_data.ndim != 3:
        raise ValueError(
            f'EEG classification data must be 3D [N, T, C], got '
...
def load_EEG_cls_split(dataset):
    dataset_path = os.path.join('eeg_processing', 'processed', f'{dataset}.npz')
    if not os.path.exists(dataset_path):
        raise FileNotFoundError(
            f'EEG dataset file not found: {dataset_path}. '
    ...
    with np.load(dataset_path, allow_pickle=False) as data:
        ...
        if 'valid_data' in data and 'valid_labels' in data:
            split['valid_data'] = data['valid_data'].astype(np.float32)
            split['valid_labels'] = data['valid_labels'].astype(np.int64)
        if 'label_names' in data:
            split['label_names'] = data['label_names']
        if 'channels' in data:
...
def _get_time_features(dt):
    iso_week = dt.isocalendar().week.to_numpy()
    return np.stack([
        dt.minute.to_numpy(),
        dt.hour.to_numpy(),

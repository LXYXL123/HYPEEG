...
class HyperbolicRelationBranch(nn.Module):
    ...
    def _pool_tokens(self, h: torch.Tensor) -> torch.Tensor:
        if h is None:
            return None
        if h.size(1) <= self.tokens_per_source:
            return h
    ...
    def forward(self, h_raw: Optional[torch.Tensor], band_features: List[torch.Tensor]) -> torch.Tensor:
        ...
        if h_raw is not None:
            tokens.append(self._pool_tokens(h_raw))
        for band_feature in band_features:
            tokens.append(self._pool_tokens(band_feature))
        if not tokens:
            raise ValueError('Relation branch requires at least one token source.')

        token_set = torch.cat(tokens, dim=1)

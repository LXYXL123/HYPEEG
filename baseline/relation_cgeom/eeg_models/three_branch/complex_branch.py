...
class SingleBandComplexEncoder(nn.Module):
    def __init__(self, input_dims: int, repr_dims: int, hidden_dims: int, depth: int):
        ...
        self.output_proj = nn.Linear(repr_dims * 2, repr_dims)

    def forward(self, x_complex: torch.Tensor) -> torch.Tensor:
        x_complex = self.input_complex_fc(x_complex)
        x_complex = self.repr_dropout(self.feature_extractor(x_complex))
        return self.output_proj(x_complex)


class FilterBankComplexBranch(nn.Module):
    def __init__(
        self,
        input_dims: int,
        repr_dims: int,
        ...
        if share_band_encoder:
            self.shared_encoder = SingleBandComplexEncoder(
        ...
        if len(self.bands) > 1:
            if fusion_type == 'concat_linear':
                self.fusion_proj = nn.Linear(repr_dims * len(self.bands), repr_dims)
                self.band_gate = None
            elif fusion_type == 'gated_sum':
                self.fusion_proj = None
                self.band_gate = nn.Linear(repr_dims * len(self.bands), len(self.bands))
            else:

...
import datautils
import tasks
from ts2vec_complex_geom import TS2VecComplexGeom
from utils import (
    broadcast_object,
    cleanup_distributed,
    data_dropout,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--loader', type=str, required=True, help='UCR, UEA, eeg_cls, forecast_csv, forecast_csv_univar, anomaly, or anomaly_coldstart')
    parser.add_argument('--gpu', type=int, nargs='+', default=[0], help='One or more GPU ids used for training. Use torchrun for multi-GPU, e.g. --gpu 0 1')
    ...
    parser.add_argument('--irregular', type=float, default=0, help='The ratio of missing observations')
    ...
    if args.eval and is_main:
        if task_type == 'classification':
            ...
            raise AssertionError('Unsupported task type')

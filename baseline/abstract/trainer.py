...
class AbstractTrainer(ABC):
    ...
    def __init__(self, cfg: AbstractConfig):
        ...
        self.best_monitor_scores: Dict[str, float] = {}
    ...
    def eval_epoch(self, dataloaders: list[DataLoader], prefix: str):
        ...
        with torch.no_grad():
            ...
            if prefix == self.cfg.logging.monitor_split:
                self.last_eval_metrics = log_dict.copy()
    ...
    def _get_monitor_key(self, ds_name: str) -> str:
        return f"{ds_name}/{self.cfg.logging.monitor_split}/{self.cfg.logging.monitor_metric}"

    def _get_monitor_score(self, ds_name: Optional[str] = None) -> Optional[float]:
        if ds_name is not None:
            monitor_key = self._get_monitor_key(ds_name)
            score = self.last_eval_metrics.get(monitor_key)
            if score is None:
                logger.warning(f"Monitor metric not found: {monitor_key}")
            return score

        scores = []
        for cur_ds_name in self.ds_info.keys():
            monitor_key = self._get_monitor_key(cur_ds_name)
            score = self.last_eval_metrics.get(monitor_key)
        ...
        if not scores:
            logger.warning(
                f"No monitor metrics found for split={self.cfg.logging.monitor_split} "
                f"metric={self.cfg.logging.monitor_metric}"
            )
            return None
    ...
    def _is_better_score(self, score: float, best_score: float) -> bool:
        if self.cfg.logging.monitor_mode.lower() == "min":
            return score < best_score
        return score > best_score

    def _maybe_save_best_checkpoint(self, ds_name: Optional[str] = None):
        if not get_is_master():
            return

        score = self._get_monitor_score(ds_name)
        if score is None:
            return

        best_key = ds_name or "unified"
        prev_best = self.best_monitor_scores.get(best_key)
        if prev_best is not None and not self._is_better_score(score, prev_best):
            return

        self.best_monitor_scores[best_key] = score
        self.save_checkpoint(ds_name=ds_name, suffix="best", monitor_score=score)

        logger.info(
            f"New best checkpoint for {best_key}: "
            f"{self.cfg.logging.monitor_split}/{self.cfg.logging.monitor_metric}={score:.4f}"
        )
    ...
    def run_unified_training(self):
        ...
        for epoch in range(self.cfg.training.max_epochs):
            ...
            # Save checkpoint
            if self.cfg.logging.save_best_only:
                self._maybe_save_best_checkpoint()
            elif (epoch + 1) % self.cfg.logging.ckpt_interval == 0:
                self.save_checkpoint()

        if self.cfg.logging.save_last_checkpoint:
    ...
    def run_separate_training(self):
        ...
        for i, (ds_name, ds_config) in enumerate(self.ds_conf.items()):
            ...
            for epoch in range(self.cfg.training.max_epochs):
                ...
                # Save checkpoint
                if self.cfg.logging.save_best_only:
                    self._maybe_save_best_checkpoint(ds_name=ds_name)
                elif (epoch + 1) % self.cfg.logging.ckpt_interval == 0:
